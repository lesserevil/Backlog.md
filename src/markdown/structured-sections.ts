...
function commentsLegacyRegex(flags: string): RegExp {
	return new RegExp(
		`(\\n|^)${escapeForRegex(COMMENTS_SECTION_HEADER)}\\s*\\n([\\s\\S]*?)${structuredSectionLookahead(COMMENTS_TITLE)}`,
		flags,
	);
}
...
function parseComments(content: string): TaskComment[] {
	const src = content.replace(/\r\n/g, "\n");
	const sentinelMatch = commentsSentinelRegex("i").exec(src);
...
function formatCommentBlock(comment: TaskComment, fallbackIndex: number): string {
	...
	lines.push(`created: ${String(comment.createdDate ?? "").trim()}`, "", body, COMMENT_END_MARKER);
	return lines.join("\n");
}
...
function stripCommentsSection(content: string): string {
	let stripped = content;
	stripped = stripped.replace(commentsSentinelRegex("gi"), "\n");
	stripped = stripped.replace(commentsLegacyRegex("gi"), "\n");
	return stripped.replace(/\n{3,}/g, "\n\n").trimEnd();
}
